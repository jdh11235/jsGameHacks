...
jsGameHacks.ExportSave = {
  downloadJSON: function(filename, contents) {
    var blob = new Blob([contents], {type: 'text/json;charset=utf-8'});
    saveAs(blob, filename);
  },

  exportGameState: function(filename) {
    jsGameHacks.ExportSave.downloadJSON(filename, localStorage.gameState);
  },

  createDialog: function() {
    var filename = window.prompt("Name:");
    if (filename !== null && filename !== '') {
      jsGameHacks.ExportSave.exportGameState(filename + '.2048-SAVE.json');
    }
  }
};

jsGameHacks.ExportSave.createDialog();
